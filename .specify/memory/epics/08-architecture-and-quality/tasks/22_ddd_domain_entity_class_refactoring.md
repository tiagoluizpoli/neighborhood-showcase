...
- [x] Abstract class `Entity` and `AuditableEntity` created and exported correctly.
- [x] Domain files (`condominium.entity.ts`, `announcement.entity.ts`, `assignment.entity.ts`, `payment.entity.ts`) are classes enforcing DDD invariants.
- [x] Domain constructors throw custom `DomainError` instances.
- [x] Backend tRPC router maps `DomainError` exceptions to bad-request `TRPCError` payloads.
- [x] Database repository implementations use concrete `EntityMapper` classes.
- [x] Server test suite compiles and runs successfully with green assertions.
