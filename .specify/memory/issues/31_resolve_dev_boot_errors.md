...
1. Update `INIT_ADMIN_API_TOKENS` environment setting in the `unleash` service configuration inside `docker-compose.yml` to change `default:development.unleash-insecure-api-token` to `*:development.unleash-insecure-api-token` to ensure the server starts without validation errors.
