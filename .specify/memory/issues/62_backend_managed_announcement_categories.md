...
- [ ] Backend has a category table with required category fields.
- [ ] Initial MVP categories are seeded.
- [ ] Announcements reference categories by `categoryId`, not free-text category strings.
- [ ] Public feed category filter accepts category identity from backend-managed categories.
- [ ] Frontend no longer hardcodes the full category taxonomy for the public feed.
- [ ] `Todos` is represented only as a UI "no category filter" state.
- [ ] Provider create/edit announcement flow selects from active backend categories.
- [ ] Public cards/detail pages display category labels from backend category data.
- [ ] Public quick filters use backend ordering rather than frontend hardcoded ordering.
- [ ] Implementation does not scan raw analytics events on public home/feed requests.
- [ ] Tests cover category seeding, announcement category assignment, public category filtering, and `Todos`/no-filter behavior.
