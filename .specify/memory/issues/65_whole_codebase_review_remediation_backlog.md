...
- 2026-06-05: Issue 72 continued by extracting the moderation announcements view into internal `panel/` route-family modules, reducing `panel.moderation.tsx` further while keeping suspend/reinstate behavior and route wiring unchanged. Validation stayed green under focused announcements/moderation coverage plus full `bun run test`, `bun run check-types`, and `bun run check`; the existing web chunk-size warning remains non-blocking and the residents/admin seams still remain.
- 2026-06-05: Issue 72 continued again by extracting the moderation residents view into internal `panel/` route-family modules, reducing `panel.moderation.tsx` further while keeping approve/reject behavior, proof preview, and route wiring unchanged. Validation stayed green under focused residents/moderation coverage plus full `bun run test`, `bun run check-types`, and `bun run check`; the existing web chunk-size warning remains non-blocking and the admin seams still remain.
- 2026-06-05: Issue 72 continued again by extracting the admin pending-condominiums approval surface into internal `panel/` route-family modules, reducing `panel.admin.tsx` while keeping role guards, query orchestration, proof preview behavior, and approve/reject mutation wiring unchanged. Validation stayed green under focused admin/guard coverage plus full `bun run test`, `bun run check-types`, and `bun run check`; the existing web chunk-size warning remains non-blocking and the remaining provider-directory/blacklist/users admin seams still remain.
- 2026-06-05: Issue 72 continued again by extracting the admin CPF blacklist surface into an internal `panel/` route-family module, reducing `panel.admin.tsx` further while keeping tab gating, add/remove mutation wiring, and blacklist behavior unchanged. Validation stayed green under focused admin/guard coverage plus full `bun run test`, `bun run check-types`, and `bun run check`; the existing web chunk-size warning remains non-blocking and the remaining provider-directory/users admin seams still remain.
