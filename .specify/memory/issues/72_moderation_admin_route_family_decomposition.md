...
- 2026-06-07: Iteration 6 continued the admin-side decomposition by extracting the providers directory surface into an internal `panel/` route-family module, keeping `panel.admin.tsx` responsible for tab/query/mutation wiring while the new providers panel owns header, search, provider table, and inline ban form composition; added focused providers coverage and passed `bun test apps/web/src/routes/panel/-admin-providers-panel.test.tsx`, `bun test apps/web/src/routes/-guards.test.ts`, `bun run test`, `bun run check-types`, and `bun run check` (with the existing web chunk-size warning still non-blocking).
