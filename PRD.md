...
## Module 23: Panel Layout Redesign

The authenticated panel (`/panel/*`) gets a redesigned sidebar and top bar, replacing the basic sidebar and top bar user menu from Module 8 (Item 11) with a proper nested navigation, SidebarFooter, and role-scoped visibility.
...
73. As a Provider, I want my sidebar to show nested items under "Provedor" (Dashboard, Meus Anúncios, Configurações), so that I can navigate to all my provider tools from one place.
74. As a Provider, I want to see badge counts on the "Moderação" section showing pending announcements, pending residents, and open reports (stubbed to 0 for now), so that the structure is ready for live data when the backend endpoints exist.
75. As a Provider, I want the sidebar to persist its collapsed/expanded state across reloads, so that my preferred layout is restored on return.
76. As a Provider, I want my user identity (avatar, name, email) and account actions (Conta, Sair) to appear in the sidebar footer, so that I can access my account without looking in the top bar.
77. As a Provider, I want a theme toggle in the top bar that switches between light and dark mode, so that I can work in my preferred visual theme.
78. As a Provider, I want a language toggle in the top bar that switches between Portuguese and English via a flag popover, so that I can use the platform in my preferred language.
79. As a Provider, I want all sidebar labels and UI text to be localized, so that the platform is fully available in both Portuguese and English.
...
87. As a Provider who opts out during onboarding, I never want a Provider Assignment created, so that the Provedor section never appears for me.
...
## Module 24: Panel i18n, Navigation Hierarchy & Moderation Condo Context
...
3. **Wrong navigation hierarchy.** Provider group nests Announcements + Configuration inside a `SidebarMenuSub` (collapsible) while Dashboard is top-level — all three should be flat siblings. Spectrum's item is at root level instead of inside the Spectrum group.
...
92. As a panel user, I want all sidebar labels to display in my language (Portuguese or English), not raw i18n keys, so I can navigate correctly.
93. As a moderator, I want to see information about the condominium I moderate (name, address, contact), so I can verify and share it.
94. As a moderator assigned to multiple condominiums, I want to switch between them from inside the Moderation nav group, so I can work on any condo without logging in and out.
95. As a moderator assigned to a single condominium, I want to see the condo name as context inside the Moderation nav group, so I always know which condo I'm working in.
96. As a provider, I want Dashboard, Announcements, and Configuration to be equally visible as siblings in the Provider nav group, so navigation is flat and predictable.
97. As an administrator, I want the Spectrum item to appear only inside the Spectrum group, not at the top level of the sidebar.
98. As a developer, I want every UI change to have a Playwright test, so regressions are caught before delivery.
...
**Backend:** New tRPC procedure `getCondominiumInfo` in a new `condominium` router. Input: `{ condominiumId: string }`. Output: full condominium object. Authorization: caller must have an approved MODERATOR or RESIDENT assignment for that `condominiumId`.
...
- Zero condos: selector does not render
...
#### 8. Playwright setup (web app)

Install Playwright in `apps/web/`. Configure `playwright.config.ts`. Create `tests/` directory scaffold with a smoke test that verifies the sidebar renders with visible text (not raw i18n keys). Add `test:e2e` script to `package.json`. This unblocks all UI epics.

#### 9. English-only code paths and i18n key naming

All code — file names, variable names, function names, route paths, and i18n key paths — must be in English. Translation values remain in their respective languages (Portuguese, English). This rule applies everywhere in the codebase, including i18n key paths (e.g., `sidebar.group.spectrum`, not `sidebar.group.espectro`).
...
- **Spectrum hierarchy test:** Playwright asserts Spectrum item is NOT a top-level sidebar item and IS visible inside the Spectrum group.
- **All UI tasks:** Every task touching frontend code must add or update a Playwright test. If a test fails, the AI self-corrects before presenting the result.
