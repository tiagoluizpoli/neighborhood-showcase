...
export interface AnnouncementDashboardCardData {
  id: string;
  imageUrl: string;
  title: string;
  description: string;
  ...
  status: 'ACTIVE' | 'DRAFT' | 'EXPIRED' | 'PENDING_PAYMENT' | 'SUSPENDED';
...
const STATUS_LABEL: Record<AnnouncementDashboardCardData['status'], string> = {
  ACTIVE: 'Ativo',
  DRAFT: 'Rascunho',
  EXPIRED: 'Expirado',
...
const STATUS_CLASS: Record<AnnouncementDashboardCardData['status'], string> = {
  ACTIVE: 'border border-success/30 bg-success/20 text-success',
  DRAFT: 'border bg-secondary text-secondary-foreground',
  EXPIRED: 'border border-destructive/30 bg-destructive/20 text-destructive',
  PENDING_PAYMENT: 'border border-warning/30 bg-warning/20 text-warning',
  SUSPENDED: 'border border-destructive/40 bg-destructive/30 text-destructive',
};
...
export function AnnouncementDashboardCard({
  ...
  const statusLabel =
    ad.status === 'ACTIVE' && ad.flaggedForReview
      ? 'Ativo (Em revisão)'
    ...
    <div className="flex flex-col overflow-hidden rounded-2xl border bg-card text-card-foreground shadow-sm">
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-muted">
        ...
        <div className="absolute top-3 right-3 flex flex-col items-end gap-1.5">
          <span
            className={`rounded-full px-2.5 py-1 font-semibold text-xs ${STATUS_CLASS[ad.status]}`}
          >
            {statusLabel}
          </span>
